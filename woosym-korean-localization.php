...
define( 'WSKL_VERSION', '3.2.6-alpha1' );
...
if ( ! function_exists( 'wskl_plugin_monitor' ) ) :
	...
	function wskl_plugin_monitor() {
		...
		/** 우커머스 비할성화 시 알림.*/
		wskl_add_plugin_status( 'woocommerce/woocommerce.php',
		                        'inactive',
		                        array(
			                        'WSKL_Plugins_React',
			                        'woocommerce',
		...
		/** SYM-MVC 활성화 시 대응 */
		wskl_add_plugin_status( 'sym-mvc-framework/sym-mvc-framework.php',
		                        'active',
		                        array(
			                        'WSKL_Plugins_React',
		...
		/** 아임포트 활성화 시 대응 */
		wskl_add_plugin_status( 'iamport-for-woocommerce/IamportPlugin.php',
		                        'active',
		                        array(
			                        'WSKL_Plugins_React',
			                        'iamport_plugin',
...
function wskl_startup_plugin() {
	...
	if ( is_admin() ) {
		add_action( 'admin_enqueue_scripts',
			function () {
				...
				wp_enqueue_style( 'wskl-admin-css',
	...
	if ( ! function_exists( 'wskl_plugin_add_settings_link' ) ) {
		...
		function wskl_plugin_add_settings_link( $links ) {
			...
			if ( isset( $links['0'] ) && FALSE !== strstr( $links[0],
			                                               'Settings' )
			) {
				unset( $links[0] );
			}

			$dabory_url        = add_query_arg( 'page',
			                                    WSKL_MENU_SLUG,
			                                    admin_url( 'admin.php' ) );
			$settings_link     = wskl_html_anchor( __( 'Settings' ),
