...
final class Woosym_Korean_Localization_Settings extends Sym_Mvc_Settings {
	...
	public function add_menu_item() {  // Add settings page to admin menu
		...
		$this->setting_menu_hook = add_menu_page(
			__( '다보리', 'wskl' ), __( '다보리', 'wskl' ), 'manage_options', WSKL_MENU_SLUG, array(
			$this,
			'settings_page',
		), 'dashicons-cart', 56
		);
	...
	private function settings_fields() {  // specify settings fields
		...
		if ( isset( $_GET['tab'] ) && $_GET['tab'] == 'authentication' ) {
			...
			$payment_description = sprintf(
				...
				__( '지불기능 키를 입력후 기능을 활성화하십시오.', 'wskl' ), __( '지불기능 인증', 'wskl' ),
			...
			$essential_description = sprintf(
				...
				__( '핵심기능 키를 입력후 기능을 활성화하십시오.', 'wskl' ), __( '핵심기능 인증', 'wskl' ),
			...
			$extension_description = sprintf(
				...
				__( '확장기능 키를 입력후 기능을 활성화하십시오.', 'wskl' ), __( '확장기능 인증', 'wskl' ),
			...
			$marketing_automation_description = sprintf(
				...
				__( '마케팅자동화 키를 입력후 기능을 활성화하십시오.', 'wskl' ), __( '마케팅자동화 인증', 'wskl' ),
			...
			case 'ags':
				array_push(
					...
					array(
						...
						'description' => __(
							'결제페이지 스크립트을 HTTPS(보안모드)방식으로 호출합니다.<br/>
		...
		$settings['checkout-shipping'] = array(
			...
			'fields'      => array(
				...
				array(
					...
					'description' => __(
						'관련상품에 표시되는 갯수를 결정합니다.<font color="red">(테마에 따라 적용되지 않는 경우도 있으므로 유의하세요.)</span>', 'wskl'
					),
					'type'        => 'shorttext',
					'default'     => '4',
				...
				array(
					...
					'description' => __(
						'테마나 타 플러그인에서 관련상품 값을 덮어쓸 수 있습니다. 만약 원하는 결과가 나오지 않으면 이 숫자를 늘려서 우선 순위를 낮춰 보세요', 'wskl'
					),
					'type'        => 'shorttext',
					'default'     => '99',
		...
		if ( get_option( WSKL_PREFIX . 'fb_login' ) == 'on' ) {
			array_push(
				...
				'placeholder' => '',
					...
					'description' => __(
						'로그인 링크에 보여질 텍스트 또는 이미지 태그를 입력하십시오. 기본 아이콘을 사용하려면 \'[icon]\'으로 입력하세요.', 'wskl'
					),
					'type'        => 'textarea',
					'default'     => '[icon]',
					'placeholder' => '',
