...
class Sym_Social_Login  {
    ...
    public function __construct() {
        ...
        if (get_option( $woo_sym_prefix . 'fb_login') == 'on' )
            add_action('init', array($this, 'service_social_login_fb'), 10);
        if (get_option( $woo_sym_prefix . 'naver_login') == 'on' )
    ...
    function add_social_login() {
        ...
        ';
        if (get_option( $woo_sym_prefix . 'fb_login') == 'on' )
            $html .= '<a href="/index.php?sym-api=service-social-login-fb">'. get_option($woo_sym_prefix . 'fb_login_link_title'). '</a> ';

        if (get_option( $woo_sym_prefix . 'naver_login') == 'on' )
            $html .= '<a href="/index.php?sym-api=service-social-login-naver">'. get_option($woo_sym_prefix . 'naver_login_link_title'). '</a> ';
    ...
    function service_social_login_fb() {
        ...
        if  (  (isset($_GET['sym-api']) &&  $_GET['sym-api'] == 'service-social-login-fb')  ) {
            add_option( $woo_sym_prefix. 'sym-api-'. $_SERVER['HTTP_CLIENT_IP'], 'service-social-login-fb' );
        }

        if  (  (isset($_GET['sym-api']) &&  $_GET['sym-api'] == 'service-social-login-fb')
                    || (isset($_GET['code']) && get_option($woo_sym_prefix. 'sym-api-'. $_SERVER['HTTP_CLIENT_IP']) == 'service-social-login-fb' ) ) {
            ...
            $client->client_id = get_option($woo_sym_prefix . 'fb_app_id');
            $client->client_secret = get_option( $woo_sym_prefix . 'fb_app_secret');
    ...
    function service_social_login_naver() {
        ...
        if  (  (isset($_GET['sym-api']) &&  $_GET['sym-api'] == 'service-social-login-naver')  ) {
            add_option( $woo_sym_prefix. 'sym-api-'. $_SERVER['HTTP_CLIENT_IP'], 'service-social-login-naver' );
        }

        if  (  (isset($_GET['sym-api']) &&  $_GET['sym-api'] == 'service-social-login-naver')
            || (isset($_GET['code']) && get_option($woo_sym_prefix. 'sym-api-'. $_SERVER['HTTP_CLIENT_IP']) == 'service-social-login-naver' ) ) {
            ...
            $client->client_id = get_option($woo_sym_prefix . 'naver_client_id');
            $client->client_secret = get_option( $woo_sym_prefix . 'naver_client_secret');
    ...
    function process_social_login() {
        ...
        if (isset($_GET['sym-api']) AND  $_GET['sym-api'] == 'process-social-login' ) {
           ...
           // sym__alert($social_code. '==='.  $social_id. '==='.  $user_name. '==='.  $user_nick. '==='.  $user_email. '===' );
            ...
            delete_option( $woo_sym_prefix. 'sym-api-'. $_SERVER['HTTP_CLIENT_IP'] ); //사용했던 옵션 반드시 삭제해야 함.
