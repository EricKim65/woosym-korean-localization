...
class WC_Kcp_Common extends WC_Payment_Gateway {
  ...
  public function __construct() {
    ...
    $this->method_title       = $payment_gate_names[ 'kcp' ] . " - {$guided_methods[ $this->method ]}";
  ...
  function init_form_fields() {  // action에 포함되는 것이므로 include로 뺄수 없슴.
    $this->form_fields = array(
        'enabled'     => array(
            ...
            'type'    => 'checkbox',
            'label'   => __( '해당 결제방법을 활성화합니다.<br/>
                                    활성화 후에 상단에 있는 [결제 옵션]-[지불게이트 웨이]메뉴에서 [회원 결제 페이지]의 표시 순서를 조정하십시요. ', $this->_folder ),
            'default' => 'yes',
        ),
        'title'       => array(
  ...
  function order_pay_form( $order_id ) {
    ...
    require_once dirname( __FILE__ ) . '/noedit_kcppay.php';
  ...
  function pay_form_input_args( $order ) {
    ...
    $pay_form_args = array(  // 공통 변수
      // 주문정보 입력
        'ordr_idxx'             => $order_id,
        'pay_method'            => $this->method_code,
        'good_name'             => $item_name,  // 상품명
        ...
        'buyr_tel2'             => $order->billing_phone, //휴대폰번호, - 포함  똑같음.

      //가맹점 필수정보
        'req_tx'                => 'pay',
        'site_cd'               => $this->kcp_sitecd,
        'site_name'             => $this->kcp_sitename,
        'quotaopt'              => '12',
        'currency'              => 'WON',
  ...
  function process_payment( $order_id ) {
    ...
    if ( $woocommerce_ver21_less ) {
      $return_array = array( 'result'   => 'success',
  ...
  function order_received_text() {
    ...
    if ( isset( $_POST["req_tx"] ) ) { // PG가 연결되었을때만 (무통장입금이 아닐때)
      /* = -------------------------------------------------------------------------- = */
      /* =   PAGE : 결과 처리 PAGE                                                    = */
      /* = -------------------------------------------------------------------------- = */
...
class Kcp_Pay_Callback extends WC_Payment_Gateway {
  ...
  function pay_callback() {
    ...
    $g_conf_home_dir = dirname( __FILE__ ) . '/homekcp/';        // BIN 절대경로 입력 (bin전까지)
