...
class WC_Inicis_Common extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_thankyou_order_received_text',
		            array( $this, 'order_received_text' ), 1, 0 );
	...
	function order_pay_form( $order_id ) {
		...
		* ************************** */
		 ...
		 * 3. 암호화 대상/값 설정 *
		 * ************************ */
		$inipay->SetField( "inipayhome",
		                   dirname( __FILE__ ) . '/homeinicis/' );       // 이니페이 홈디렉터리(상점수정 필요)
		$inipay->SetField( "type", "chkfake" );      // 고정 (절대 수정 불가)
		$inipay->SetField( "debug",
		                   "true" );        // 로그모드("true"로 설정하면 상세로그가 생성됨.)
		$inipay->SetField( "enctype",
		                   "asym" );    //asym:비대칭, symm:대칭(현재 asym으로 고정)
		/* * ************************************************************************************************
		 ...
		 * ************************************************************************************************ */
		$inipay->SetField( "admin",
		                   $this->inicis_admin );     // 키패스워드(키발급시 생성, 상점관리자 패스워드와 상관없음)
		$inipay->SetField( "checkopt",
		                   "false" );   //base64함:false, base64안함:true(현재 false로 고정)
		...
		$inipay->SetField( "nointerest",
		                   "no" );             //무이자여부(no:일반, yes:무이자)
		...
		// 세션 정보를 사용하지 않고, 사용자의 usermeta 필드를 이용한다.
		$inicis_session_vars = array(
			//상점ID
			'INI_MID'     => $this->inicis_mid,

			// 키패스워드(키발급시 생성, 상점관리자 패스워드와 상관없음)
			'INI_ADMIN'   => $this->inicis_admin,

			//가격
			'INI_PRICE'   => $order->order_total,

			//고정 (절대 수정 불가)
			'INI_RN'      => $inipay->GetResult( "rn" ),

			//고정 (절대 수정 불가)
			'INI_ENCTYPE' => $inipay->GetResult( "enctype" ),

			// 셋업 정보를 pay-callback으로 넘기기위해서
			'inicis_url'  => $this->inicis_url,
		);
	...
	function pay_form_input_args( $order ) {
		...
		$pay_form_args = array(  // 공통 변수
		                         ...
		                         // 주문자 이메일
		                         'parentemail'  => $order->billing_email,
		                         // 주문자 이메일 - 정통부 권고 사항
	...
	function js_and_css() {
		...
		if ( $this->enable_https != 'on' ) {
			wp_enqueue_script( 'inicis_wallet',
			                   'http://plugin.inicis.com/pay61_secuni_cross.js' );
		} else {
			wp_enqueue_script( 'inicis_wallet',
			                   'https://plugin.inicis.com/pay61_secunissl_cross.js' );
		}

		wp_enqueue_script( 'inicis_pay',
		                   $this->assets_url . 'js/inicispay.js' );  //맨앞에 넣음
		// 이걸 빼면 안됨
	}
...
class Inicis_Pay_Callback extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_api_inicis_pay_callback',
	...
	function pay_callback() {
		...
		*/
		...
		$inicis_session_vars = (array) get_user_meta( get_current_user_id(),
		                                              wskl_get_option_name( 'inicis_session_vars' ),
		                                              TRUE );
		...
		*/
		...
		if ( $inipay->GetResult( 'ResultCode' ) == "00" ) {
			$order->add_order_note( sprintf( __( '결제가 성공적으로 처리됨.<br/>결제방법: %s<br/>이니시스 TID: %s. 발생시각: %s.',
			                                     'wskl' ), $this->method, '111',
			                                 date( 'Y-m-d H:i:s' ) ) );
			...
			$res_msg = iconv( 'euc-kr', 'utf-8',
			                  $inipay->GetResult( 'ResultMsg' ) );
			$order->update_status( 'failed',
			                       sprintf( __( '결제처리 안됨.<br/>-에러메시지 : %s<br/>-발생시각: %s.',
			                                    'wskl' ), $res_msg,
			                                date( 'Y-m-d H-i-s' ) ) );
